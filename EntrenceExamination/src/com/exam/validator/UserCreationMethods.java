...
//				if (builder.toString().trim() != "" && builder.charAt(j) == '.' || builder.charAt(j) == ' '
//						|| (builder.charAt(j) >= 'A' && builder.charAt(j) <= 'z')) {
//					continue;
//
//				} else {
//					builder.setLength(0);
...
//			}
			...
			boolean matchFound=Pattern.compile("^[A-Za-z\\s]{1,}[\\.]{0,1}[A-Za-z\\s]{0,}$",Pattern.CASE_INSENSITIVE).matcher(builder.toString()).find();
			if(matchFound) {
				isExit = false;
			} else {
				builder.setLength(0);
...
//				System.out.println();
//			}
			boolean matchFound = Pattern.compile("BC|OC|SC|ST", Pattern.CASE_INSENSITIVE).matcher(builder.toString()).find();
			if (matchFound) {
				isExit = false;
			} else {
				builder.setLength(0);
...
//			}
			...
			boolean matchFound = Pattern.compile("[6-9]{1}[0-9]{9}").matcher(builder.toString()).find();
			if (matchFound) {
				isExit = false;
			} else {
				isExit = true;
