			...
			try {
				...
				if (myAssigendTickets.size() > 1) {
					...
					for (Entry<Integer, TicketDetails> entry : myAssigendTickets.entrySet()) {
						...
						if ("inprogress".equals(details.getTicket_Status())) {

							out.print("<tr id=" + entry.getKey() + " value=" + entry.getKey()
									+ " style='cursor: pointer;" + "text-align: center;'><td>" + entry.getKey()
									...
									+ ")'>Change Admin</button><td> <button onclick='ticketCloseFunction("
									+ entry.getKey() + ")'>Close Ticket</button><td><button onclick='getIdFunction("
									+ entry.getKey() + "	)'>Chat</button></td>");
