...
/**
 * Servlet implementation class GetClosedTickets
 */
@WebServlet("/GetClosedTickets")
public class GetClosedTickets extends HttpServlet {
				...
				for (Entry<Integer, TicketDetails> entry : userTickets.entrySet()) {
					...
					if ("closed".equals(details.getTicket_Status())) {
						...
						out.print("<tr id=" + entry.getKey() + " value=" + entry.getKey() + " style='cursor: pointer;"
								...
								+ "<td> <button onclick='ticketTrashFunction(" + entry.getKey()
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (httpSession.getAttribute("UserId") != null  && request.getParameter("ticketId") != null) {
			int ticketId=Integer.parseInt(request.getParameter("ticketId"));
			 ...
			 try {
				new Userdetails().supportRequestStatusChanger(ticketId , 2);
